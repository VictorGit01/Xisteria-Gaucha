...
const Header = styled.View`
    ...
    padding-vertical: ${normalize(30)}px;
...
const BoxSquare = styled.View`
    ...
    border-radius: ${props => props.bdRadius || normalize(3)}px;
...
const Screen = () => {
    ...
    function getOpeningHours() {
        if (currentCity) {
            ...
            cities.child(cityId).child('open').on('value', snapshot => {
                let newOpen = snapshot.val()
                setOpen(newOpen)
            })
        }
    }
    ...
    function toggleOpening() {
        if (currentCity) {
            ...
            cities.child(cityId).child('open').set(!open)
                .then(() => {
                    setTimeout(() => {
                        setOpen(!open);
    ...
    function handleSave() {
        if (currentCity) {
            ...
            if (filteredDaysP1.length < 1) {
                ...
                setLoaderVisible(false)
                // toastMsg('É preciso inserir todos os horários do 2º período ao definir pelo menos um horário no mesmo')
    ...
    function handleCopyAboveP1(index) {
        let schedulesP1Copy = Array.from(schedulesP1)
        console.log('-------------SCHEDULESP1 - BEGIN-------------')
        console.log(schedulesP1[index - 1].date_beg)
    ...
    function handleCopyAboveP2(index) {
        let schedulesP2Copy = Array.from(schedulesP2)
        console.log('-------------SCHEDULESP2 - BEGIN-------------')
        console.log(schedulesP2[index - 1].date_beg)
    ...
    return (
        <Page>
            ...
            >
                <Header>
                    <ButtonAlt onPress={toggleOpening} >
                        <FontIcon 
                            name={open ? 'toggle-on' : 'toggle-off'} 
                            size={normalize(25)} color={open ? 'green' : 'red'}  
                        />
                    </ButtonAlt>
                    <ItemText style={{ fontWeight: 'bold', left: normalize(30) }} >Local {open ? 'Aberto' : 'Fechado'}</ItemText>
                </Header>
                ...
                {schedulesP2.map((item, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* <BoxSquare></BoxSquare> */}
                            <LeftItem>
                                {/* <BoxSquare selected={selectedDaysP2.includes(index) ? true : false} bdRadius={12} ></BoxSquare> */}
                                <Icon name={selectedDaysP2.includes(index) ? 'check' : 'close'} size={normalize(25)} color='#fe9601' style={{ marginLeft: normalize(5), marginRight: normalize(15) }} />
                                <ItemText>{item.day}</ItemText>
                            </LeftItem>
